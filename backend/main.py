...
async def upload_dataset(file: Annotated[UploadFile, File(...)]) -> dict:
    ...
    if not any(filename.endswith(ext)
        ...
        raise HTTPException(status_code=400,
                            detail="Неверный формат файла. Допустим: zip")
    data_dir = os.path.join("data")
    os.makedirs(data_dir, exist_ok=True)
    file_path = os.path.join(data_dir, filename)
    content = await file.read()
    if len(content) > 800 * 1024 * 1024:
        logger.error("Размер файла превышает 800 МБ")
...
def get_experiments() -> dict:
    ...
    """
    res = training.list_experiments()
    result = []
    for elem, metrics_and_params in res.items():
        entry = {"id": elem, "metrics": metrics_and_params[0],
                 "params": metrics_and_params[1]}
        result.append(entry)
